-- credit: uglyburger0 for the audio data and rough module layout
...
-- singleton state for if the sounds are instanciated
local isLoaded = false
local loadedSounds: {[string]: {Sound}}
local loadedMap: {[Enum.Material]: {Sound}}
local loadedSoundsFolder: Folder
local providerRandom = Random.new()

-- instanciates and loads the sounds into a datastructure for easy access
function main:loadFootstepSounds(parent: Instance, properties: {[string]: any})
	if isLoaded then
		return loadedSounds
	end
	-- create table to store sound instances
	loadedSounds = {}
	loadedMap = {}
	-- create folder to store sound instances in game (for possible future usecases)
	loadedSoundsFolder = Instance.new("Folder")
	loadedSoundsFolder.Name = "FootstepSounds"
	for footstepGroupId, footstepGroup in pairs(main.SoundIds) do
		-- create table and folder instance for sound group
		local groupList = {}
		loadedSounds[footstepGroupId] = groupList
		local groupFolder = Instance.new("Folder")
		groupFolder.Name = footstepGroupId
		for _, soundId in ipairs(footstepGroup) do
			-- create sound instance given sound id
			local sound = Instance.new("Sound")
			sound.SoundId = soundId
			sound.Playing = false
			sound.Name = footstepGroupId .. "_FootstepSound"
			for propertyName, propertyValue in pairs(properties) do
				local succ, err = pcall(function()
					sound[propertyName] = propertyValue
				end)
				if not succ then
					warn(string.format("FootstepProvider can't set property %s of sound to %s", propertyName, tostring(propertyValue)))
				end
			end
			sound.Parent = groupFolder
			table.insert(groupList, sound)
		end
		groupFolder.Parent = loadedSoundsFolder
	end
	-- create a map of materials to sound lists
	for material, footstepGroup0 in pairs(main.MaterialMap) do
		for footstepGroupId, footstepGroup1 in pairs(main.SoundIds) do
			if footstepGroup0 == footstepGroup1 then
				loadedMap[material] = loadedSounds[footstepGroupId]
				break
			end
		end
	end
	loadedSoundsFolder.Parent = parent
	isLoaded = true
	return loadedSounds
end

-- returns a random sound instance given an input material (does not return the same sound consecutively)
function main:getRandomFootstepSound(material: Enum.Material)
	if not isLoaded then
		error("FootstepProvider not loaded, can not get sound.")
	end
	local footstepGroup = loadedMap[material]
	if not footstepGroup then
		error("FootstepProvider can not get sounds for material: " .. material.Name)
	end
	local index = providerRandom:NextInteger(1, math.max(#footstepGroup - 1, 1))
	local sound = footstepGroup[index]
	footstepGroup[index] = footstepGroup[#footstepGroup]
	footstepGroup[#footstepGroup] = sound
	return sound
end
