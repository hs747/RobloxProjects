...
local FootstepProvider = require(game:GetService("ReplicatedStorage"):WaitForChild("Source"):WaitForChild("Client"):WaitForChild("FootstepProvider"))
...
local FOOTSTEP_CONFIG = {
	BasePeriod = 0.5, -- steps/second
	BasePeriodSpeedModifier = 1/16-- * second = steps/second  
}

FootstepProvider:loadFootstepSounds(game.ReplicatedStorage, {
	Volume = 0.3,
...
local function initializeSoundSystem(instances)
	local player: Player = instances.player
	...
	-- update for running using the custom sound system
	local sinceStep = 0
	local runningUpdater = function(dt: number, vel: Vector3, state: Enum.HumanoidStateType)
		local isRunningState = state == Enum.HumanoidStateType.RunningNoPhysics or state == Enum.HumanoidStateType.Running
		local flatSpeed = (vel * Vector3.new(1, 0, 1)).Magnitude
		local isMoving = flatSpeed > 0.5 and humanoid.MoveDirection.Magnitude > 0.5 -- same as the original condition
		if isRunningState and isMoving then
	...
	local steppedConn = RunService.Stepped:Connect(function(_, worldDt: number)
		...
		-- update footsteps
		runningUpdater(worldDt, rootPart.AssemblyLinearVelocity, activeState)
	end)
